...
case class BooleanMinimize(termCombinerFunc:List[List[BoolTypedTerm]] => BoolTypedTerm) extends IntegerArithmetics with Core
{
  import psksvp.ADT.Cache
  private val cache = Cache[(Set[Int], Set[BoolTypedTerm]), BoolTypedTerm]
                      {
  ...
  def apply(minTerms:List[Int],
               predicates:List[BoolTypedTerm]):BoolTypedTerm = cache((minTerms.toSet, predicates.toSet))
  ...
  def minimize(minTerms:List[Int],
               predicates:List[BoolTypedTerm]):BoolTypedTerm =
  {
    ...
    def toTerms(sl:String):Seq[BoolTypedTerm] =
    {
  ...
  /**
    ...
    * @return
    */
  def espresso(minTerms:Seq[Int], numberOfBits:Int, timeout:Int = 2, exePath:String = "espresso"):String=
  {
    ...
    /*
       ...
       .e indicates the end of input and output, .i is the number of inputs and .o is the number of output.
      ...
      The output is below, which indicates [[!p1, !p2], [p2, p3]]  from -00- and --11
...
{
  override def apply(minTerms:List[Int], predicates:List[BoolTypedTerm]):BoolTypedTerm = super.apply(minTerms, predicates)
}
