...
object SkinkExecutor
{
  ...
  def consoleRunLLVM(filename:String,
                     ...
                     predicates:Seq[BoolTypedTerm]=Nil):Unit=
  {
    PredicatesAbstraction.setToUsePredicates(predicates)
    import au.edu.mq.comp.skink.Main
  ...
  /**
    ...
    * @return
    */
  def runBench(runDataList:Seq[Code], timeout:Duration, outputDir:String, usePredAbs:Boolean=true):Seq[Seq[String]] =
  {
    ...
    for(d <- runDataList) yield
    {
      ...
      val result = runAsProcess(d.filePath, Nil, d.useO2, usePredAbs, timeout, d.useClang, d.maxIteration)
                            ...
                            maxIteration:Int=20):Unit =
  {
    ...
    runBenchAndOutputReport(name, codes, timeOut, outputDir, usePredAbs)
  }
                              ...
                              usePredAbs:Boolean=true):Unit =
  {
    ...
    val output = runBench(runDataList, timeout, subOutputDir, usePredAbs)
