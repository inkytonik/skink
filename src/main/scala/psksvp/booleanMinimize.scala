...
object booleanMinimize2 {
            ...
            def primeImplicantOf(group : Group, checkWithGroup : Group) : Set[Implicant] =
                {
                    val r = for (
                        s <- group.values;
                        t <- s if false == check(t, wasCombinedIntoGroup = checkWithGroup)
                    ) yield t
    ...
    def combination[U, V](
        lsA : Set[U],
        lsB : Set[V]
    ) : Seq[(U, V)] = (for (a <- lsA; b <- lsB) yield (a, b)).toSeq
