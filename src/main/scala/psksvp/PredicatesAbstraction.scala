...
object PredicatesAbstraction {
    ...
    class DNFComposer extends TermComposer {
        def combinationToTerm(
            combination : Int,
            predicates : Seq[BooleanTerm]
        ) : BooleanTerm = combinationToConjunctTerm(combination, predicates)

        def gamma(
            absDomain : AbstractDomain,
            predicates : Seq[BooleanTerm],
            simplify : Boolean
        ) : BooleanTerm = gammaDNF(absDomain, predicates, simplify)
    }
    ...
    class CNFComposer extends TermComposer {
        def combinationToTerm(
            combination : Int,
            predicates : Seq[BooleanTerm]
        ) : BooleanTerm = combinationToDisjunctTerm(combination, predicates)

        def gamma(
            absDomain : AbstractDomain,
            predicates : Seq[BooleanTerm],
            simplify : Boolean
        ) : BooleanTerm = gammaCNF(absDomain, predicates, simplify)
    }
 ...
 * @param termComposer
 */
case class PredicatesAbstraction(
    ...
    termComposer : PredicatesAbstraction.TermComposer
        ...
        with Disposable {
    import scala.collection.parallel.immutable.ParVector
    val solverArray = Array.fill[SMTSolver](traceAnalyzer.length)(new SMTSolver(solverFromName("Z3")))
    ...
    lazy val tracePredicates : Seq[BooleanTerm] =
        {
            ...
            val result = FixedPoint(
                equalTest,
                computePredicates
            ).run(True() :: List.fill(traceAnalyzer.length - 1)(False()))
            ...
            def nextPredicateAtLocation(loc : Int) : BooleanTerm =
                {
                    ...
                    val absPosts = for (t <- traceAnalyzer.transitionMap(loc)) yield {
                        ...
                        val absDom = for (
                            c <- List.range(0, combinationSize) if checkCombination(c, t, usePredicates)
                        ) yield c
