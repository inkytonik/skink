...
import psksvp.SMTLIB.QuantifiedTerm


/**
  * Created by psksvp on 9/5/17.
  */
object ScratchPad extends org.bitbucket.franck44.scalasmt.theories.ArrayExInt
                  with org.bitbucket.franck44.scalasmt.theories.ArrayExOperators
                  with org.bitbucket.franck44.scalasmt.typedterms.QuantifiedTerm
{
  def implies(a:BoolTypedTerm, b:BoolTypedTerm):BoolTypedTerm = !a | b

  def main(args:Array[String]):Unit = 
  {
    pphGoodSimpleTest()
    //pphBadSimpleTest()
    //pphGoodLoopAssertTest()
    //pphBadLoopAssertTest()
    //pphGoodSeqLoopAssertTest() //
    //pphBadSeqLoopAssertTest()

  }

  def pphBadSeqLoopAssertTest():Unit=
  {
    val code = inlineVerifierAssert(
                                     """
    ...
    val inv = QuantifiedTerm.Forall(j.symbol, k.symbol)(implies(j >= 0 & j < i &
      k >= 0 & k < i &
      j > k ,           a(j) > a(k)))
  ...
  def pphGoodSeqLoopAssertTest():Unit=
  {
    val code = inlineVerifierAssert(
                                     """
                                       |extern void __VERIFIER_error() __attribute__ ((__noreturn__));
  ...
  def pphBadLoopAssertTest():Unit=
  {
    ...
    val inv = QuantifiedTerm.Forall(j.symbol)(implies(j >= 0 & j < i, a(j) === 0))
  ...
  def pphGoodLoopAssertTest():Unit=
  {
    val code = inlineVerifierAssert(
                                     """
                                       |extern void __VERIFIER_error() __attribute__ ((__noreturn__));
  ...
  def pphBadSimpleTest():Unit=
  {
    ...
    val inv = QuantifiedTerm.Forall(j.symbol)(implies(j >= 0 & j < i, a(j) === 1))
  ...
  def pphGoodSimpleTestNElements():Unit=
  {
    val code = inlineVerifierAssert(
                                     """
                                       |extern void __VERIFIER_error() __attribute__ ((__noreturn__));
  ...
  def aphSkinkNaming():Unit=
  {
    ...
    val inv0 = forall(j.symbol)(implies(j >= 0 & j < i0, a(j) === 1))
    val inv1 = forall(j.symbol)(implies(j >= 0 & j < i1, a1(j) === 1))

    implicit val solver = new SMTSolver("Z3")
    val ck0 = checkPost(True(), i0 === 0, inv0)
    val ck1 = checkPost(inv0, i0 < 1000 & a1 === a.store(i0, 0) & i1 === i0 + 1, inv1)
    val ck2 = checkPost(inv0, i0 >= 1000 & !(a(10) === 1), False())
  ...
  def aphUsingStoreUpdate():Unit=
  {
    ...
    implicit val solver = new SMTSolver("Z3")
    val ck0 = checkPost(True(), i0 === 0 & i0 < 1000, inv0)
    val ck1 = checkPost(inv0, a1 === a.store(i0, 1) & i1 === i0 + 1 & i1 < 1000, inv1)
    val ck2 = checkPost(inv0, i0 >= 1000 & !(a(10) === 1), False())
  ...
  def aphTest():Unit=
  {
    ...
    val inv0 = forall(j.symbol)(implies(j >= 0 & j < i0, a(j) === 0))
    val inv1 = forall(j.symbol)(implies(j >= 0 & j < i1, a(j) === 0))

    implicit val solver = new SMTSolver("Z3")
    val ck0 = checkPost(True(), i0 === 0 & i0 < 1000, inv0)
    val ck1 = checkPost(inv0, a(i0) === 0 & i1 === i0 + 1 & i1 < 1000, inv1)
    val ck2 = checkPost(inv0, i0 >= 1000 & !(a(10) === 0), False())
  ...
  def guessingI():Unit=
  {
    ...
    val I     = implies(j >= 0 & j < i0, a0(j) === 0)
    val notI  = !(implies(j >= 0 & j < i2,  a1(j) === 0))

    val pre = n > 0 // array is not empty

    val e = pre & I & i1 === 0 & i1 < n & a1 === a0.store(i1, 0) & i2 === (i1 + 1) & i2 >= n & notI

    val solver = new SMTSolver("Z3")
    println(satisfiableCheck(e)(solver))
    solver.destroy()
  }
  ...
  def nestedLoop():Unit=
  {
    val code =
      """
        ...
        |    }
        |  }
        |  if(!(j == i)) __VERIFIER_error();
        |  return 0;
        |}
        |
      """.stripMargin

    //import org.bitbucket.franck44.scalasmt.theories.ArrayExInt
    val i = Ints("%i")
    val j = Ints("%j")
  ...
  def arrayForLoop():Unit=
  {
    ...
    SkinkExecutor.consoleRun(toFile(code),
                              List(sc === 0, sc > 0, sc < i, a(sc) === 0, i === 0, i > 0, i < size),
                              useO2 = false,
                              usePredicateAbstraction = true,
                              useClang = "clang-3.7")
  }
  ...
  def array2002():Unit=
  {
    ...
    SkinkExecutor.consoleRun(toFile(code),
                              List(sc === 0, sc > 0, sc < i, sc < spot, b(sc) === 0, i === 0, i > 0), //i < size, i < spot),
                              useO2 = false,
                              usePredicateAbstraction = true,
                              useClang = "clang-3.7",  maxIteration = 100)
  }
  

  def runTestLoopAcc():Unit=
  {
    import psksvp.SkinkExecutor._
    import scala.concurrent.duration._
    runBenchAndOutputReport("PredAbsLoopAcc", CCode.loopAcc, 60.minutes, "/home/psksvp/workspace/output", true)
    //runBenchAndOutputReport("InterpolLoopAcc", CCode.loopAcc, 5.minutes, "/home/psksvp/workspace/output", false)
  }
  ...
  def eqe():Unit =
  {
    ...
    val e = QuantifiedTerm.Exists(i.symbol, j.symbol, k.symbol)
    {
      i === x & j === y & k === (i >= j) & !k
    }
    val solver = new SMTSolver("Z3")
    ...
    Z3QE(e)(solver) match
    {
      case Success(ts) => for(t <- ts) log(termAsInfix(t))
      case _           => log("eqe fail")
    }
  ...
  def manualPredicateAbstract(): Unit =
  {
    val solver = new SMTSolver("Z3")
    val i0 = Ints("%i0")
    val i1 = Ints("%i1")
    ...
    val p0 = i1 <= 0
    val p1 = i1 >= 0
    println(termAsInfix(!p0 | !p1) + " ::: " + checkPost(pre, effect, !p0 | !p1)(solver))
    println(termAsInfix(!p0 | p1) + " ::: " + checkPost(pre, effect, !p0 | p1)(solver))
    println(termAsInfix(p0 | !p1) + " ::: " + checkPost(pre, effect, p0 | !p1)(solver))
    println(termAsInfix(p0 | p1) + " ::: " + checkPost(pre, effect, p0 | p1)(solver))
    ...
    val cmp1 = Bools("%cmp1")
    /*
    [ Or(  And( (%i@0  <= 1000 ) ,(%a@0  = %i@0 ) ) ,false ) ],                                   pre
      And( (%0@1  = %i@0 ) ,%cmp@1 ,(%cmp@1  = (%0@1  < 1000 ) ) ) ,                              effect
    [ Or(  And(  Not( (%i@0  = 1000 ) ) ,(%i@0  <= 1000 ) ,(%a@0  = %i@0 ) ) ,false ,false ) ]    Post
     */
    val pre2 = (i0 <= 1000) & (a0 === i0)
    val eff = (z1 === i0) & cmp1 & (cmp1 === (z1 < 1000))
    val post = !(i0 === 1000) & i0 <= 1000 & a0 === i0

    println("-------------------------------")
    println(checkPost(pre2, eff, post)(solver))
    println(checkPost(i0 <= 1, eff, i0 === 0)(solver)) */
