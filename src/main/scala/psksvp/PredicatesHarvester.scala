...
{
  def inferredPredicates:Set[BoolTypedTerm]
  def inferredWithFilter(f:Set[BoolTypedTerm] => Set[BoolTypedTerm]):Set[BoolTypedTerm] = f(inferredPredicates)
}
...
case class EQEPredicatesHarvester(traceAnalyzer:TraceAnalyzer,
                                  functionInformation:VariableAnalysis,
                                                       ...
                                                       with QuantifiedTerm
{
  ...
  ///
  def inferPredicates(blockNo:Int, pre:Option[BoolTypedTerm] = None):Seq[TypedTerm[BoolTerm, Term]] =
  {
    ...
    if(s.nonEmpty)
    {
      ...
      val e = SMTLIB.QuantifiedTerm.Exists(s.head, s.tail:_*)(blockEffect) //( indexedPre & blockEffect)
  ...
  override def inferredPredicates: Set[BoolTypedTerm] =
  {
