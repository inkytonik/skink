...
package object psksvp {
     ...
     * @return
     */
    def satisfiableCheck(term : BooleanTerm)(implicit solver : SMTSolver) : SatResponses =
        {
            push()
     ...
     * @return
     */
    def validityCheck(term : BooleanTerm)(implicit solver : SMTSolver) : Boolean = UnSat() == satisfiableCheck(!term)
     ...
     * @return
     */
    def equivalence(f1 : BooleanTerm, f2 : BooleanTerm)(implicit solver : SMTSolver) : Boolean = validityCheck(f1 === f2)
     ...
     */
    def checkPost(p : BooleanTerm, e : BooleanTerm, q : BooleanTerm)(implicit solver : SMTSolver) : Boolean = satisfiableCheck(p & e & !q) match {
        case Sat()   => false
        case UnSat() => true
        case _       => false
    }
    ...
    def booleanMinimize(
        minTerms : List[Int],
        predicates : List[BooleanTerm]
    ) : List[List[BooleanTerm]] =
        {
