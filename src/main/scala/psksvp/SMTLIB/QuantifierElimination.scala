...
trait QuantifierElimination {
    def apply(existsTerm : TypedTerm[BoolTerm, Term])(implicit solver : SMTSolver) : Try[Seq[TypedTerm[BoolTerm, Term]]]
}
    ...
    def apply(existsTerm : TypedTerm[BoolTerm, Term])(implicit solver : SMTSolver) : Try[Seq[TypedTerm[BoolTerm, Term]]] =
        {
            ...
            val boundedVarDeclCmd : Seq[Command] = for (v <- boundedVars) yield DeclareFunCmd(FunDecl(
                v.sMTLIB2Symbol,
                List(),
                v.sort))
