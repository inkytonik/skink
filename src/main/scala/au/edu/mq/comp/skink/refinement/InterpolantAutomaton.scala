...
trait AddBackEdges extends Core with Resources {
     ...
     * @param   choices     An infeasible path of size n in `program` given by a sequence
     *                      of choices
     ...
     * @note
     */
    def computeSafeBackEdges(program : IR, choices : Seq[Choice], preds : Seq[TypedTerm[BoolTerm, Term]]) = {
        ...
        val indexPartition : Seq[Seq[Int]] = program.traceToRepetitions(Trace(choices))
...
case class Interpolant(program : IR, choices : Seq[Choice], fromEnd : Boolean) extends AddBackEdges with Commands {
    ...
    lazy val predicateAnnotations = {
        ...
        //  of the trace
        val traceTerms = program.traceToTerms(Trace(choices))
...
object InterpolantAuto extends AddBackEdges {
    ...
    def buildInterpolantAuto(
        ...
        val predicates = Interpolant(program, choices, fromEnd).predicateAnnotations
        ...
        predicates match {
            ...
            case Success(xitp) =>
                ...
                //  compute back edges
                val newBackEdges = computeSafeBackEdges(program, choices, xitp)
