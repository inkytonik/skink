...
object AssemblyCFG extends AssemblyCFGBuilder {
    ...
    def runVerification(program : Program, cfg : CFG[FunctionDefinition, Block],
        ...
        val NameBreaker = "(.*)@([0-9]+)".r

        import au.edu.mq.comp.dot.DOTSyntax.DotSpec

        // val sinkBlock : Entry = CFGBlockEntry(Block(BlockLabel("sink"), Vector(), None, Vector(), MetaTerminatorInstruction(Unreachable(), Metadata(Vector()))))

        // // now dump the immediate post dominators
        // val postDomTree = tarjanPostDom(nfa2, "sink", sinkBlock)
        ...
        // val firstBlockEntryEdge = nfa2.edges find (x => x.lab.isBlockEntry && x.src == "%10")
        // println(firstBlockEntryEdge)

        // firstBlockEntryEdge.get.lab match {
        //     case CFGBlockEntry(b) =>
        //         val r = blockDataFlow(b, Set(Local("14")))
        //         println(r)
        //     case _ => ()
        // }

        val s1 = SlicerCFG.Slicer(nfa2)
        // val s2 = SlicerCFG.computeSlice(s1, "%7", Set(Local("i")))
        // println(s2)

        // val s3 = SlicerCFG.computeSlice(s1, "%3", Set(Local("9"), Local("3")))
        // val s4 = SlicerCFG.computeSlice(s1, "%10.aux", Set(Local("16")))
        // println(s4)

        println("full data dep with slicer method")
        val r1 = SlicerCFG.computeSlice(s1, "%10.aux", Set(Local("16")))
        println(r1)
        return
        ...
        implicit object QIdOrdering extends scala.math.Ordering[QualifiedIdentifier] {
            def compare(a : QualifiedIdentifier, b : QualifiedIdentifier) : Int =
                (a.id.symbol.name, b.id.symbol.name) match {
                    case (NameBreaker(avar, aind), NameBreaker(bvar, bind)) =>
                        val ai = aind.toInt
                        val bi = bind.toInt
                        if (ai == bi)
                            avar compare bvar
